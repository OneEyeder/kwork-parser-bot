...
async def cmd_start(message: types.Message):
    ...
    if TELEGRAM_BOT_TOKEN == 'ВАШ_ТОКЕН_ЗДЕСЬ':
        await message.answer(
            "❌ <b>Бот не настроен!</b>\n\n"
            "Пожалуйста, получите токен у @BotFather и вставьте его в файл <code>config.py</code>\n\n"
            "Инструкция:\n"
            "1. Напишите @BotFather в Telegram\n"
            "2. Отправьте /newbot\n"
            "3. Скопируйте токен и замените 'ВАШ_ТОКЕН_ЗДЕСЬ' в config.py"
        )
        return
    ...
    await message.answer(
        ...
        f"/test - тестовый запрос (проверить прямо сейчас)"
    )
...
async def main():
    ...
    # Проверяем токен
    if TELEGRAM_BOT_TOKEN == 'ВАШ_ТОКЕН_ЗДЕСЬ':
        logger.warning("Токен не настроен! Получите токен у @BotFather")
